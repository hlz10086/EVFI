...
def train(model, local_rank):
    ...
    dataset_val = Dataloader_val(args)
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(train_data):
            ...
            rgb_pair,gt,voxel_pair=data
            rgb_pair = rgb_pair.to(device, non_blocking=True)
            voxel_pair = voxel_pair.to(device, non_blocking=True)
            imgs=rgb_pair
            voxel_grid=voxel_pair
            # data_gpu = data_gpu.to(device, non_blocking=True) / 255.
            # timestep = timestep.to(device, non_blocking=True)
            #imgs = data_gpu[:, :6]
            #gt = data_gpu[:, 6:9]
            learning_rate = get_learning_rate(step) * args.world_size / 4
            pred, info = model.update(imgs, voxel_grid, gt, learning_rate=learning_rate, training=True) # pass timestep if you are training RIFEm
...
def evaluate(model, val_data, nr_eval, local_rank, writer_val):
    ...
    for i, data in enumerate(val_data):
        ...
        B, C, H, W = imgs.shape
        voxel_bins = 6
        #voxel_grid = torch.randn(B, voxel_bins, H, W, device=device)
        with torch.no_grad():
            pred, info = model.update(imgs, voxel_grid, gt, training=False)
